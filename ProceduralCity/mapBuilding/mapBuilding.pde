...
void startLoading() {
  noStroke();
  car_model = loadShape("/Assets/Objects/Cars/BasicCar.obj");
  car_model.scale(5);
  car_model.rotateX(90);
...
void generateNewMap() {
  ...
  car = new Car(cMap.getCell((int)lookAtPoint.x, (int)lookAtPoint.y), car_model, true);
...
void drawCar() {
  CompiledCell lookCell = cMap.getCell((int)lookAtPoint.x, (int)lookAtPoint.y);
  pushMatrix();
  translate(lookCell.v1.x+ROAD_WIDTH/2, lookCell.v1.y, cMap.getHeightAt(lookCell.v1.x+ROAD_WIDTH/2, lookCell.v1.y));
  lights();
  shape(car_model, 0, 0);
  noLights();
  popMatrix();
